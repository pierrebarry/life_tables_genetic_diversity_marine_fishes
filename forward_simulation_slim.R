############################################
#
#   SLiM simulation for genetic diversity
#
#############################################
...
#  for (i in line[j]:(line[j+1]-1)){
#    if (is.na(lifetime$Length[i])==TRUE){
#      lifetime$Length[i]=NA
#    } else {
#      lifetime$Length[i]=lifetime$Linf[line[j]]*(1-exp(-lifetime$K[line[j]]*(lifetime$Age[i]-lifetime$t0[line[j]])))
#    }
#  }
#}
#for (j in 1:(length(line)-1)){
#  for (i in line[j]:(line[j+1]-1)){
#    if (is.na(lifetime$Length_F[i])==TRUE){
#      lifetime$Length_F[i]=NA
#    } else #{
#      lifet#ime$Length_F[i]=lifetime$Linf_F[line[j]]*(1-exp(-lifetime$K_F[line[j]]*(lifetime$Age[i]-lifetime$t0_F[line[j]])))
    #}
  #}
#}
#for (j in 1:(length(line)-1)){
#  for (i in line[j]:(line[j+1]-1)){
#    if (is.na(lifetime$Length_M[i])==TRUE){
#      lifetime$Length_M[i]=NA
#    } else {
#      lifetime$Length_M[i]=lifetime$Linf_M[line[j]]*(1-exp(-lifetime$K_M[line[j]]*(lifetime$Age[i]-lifetime$t0_M[line[j]])))
#    }
#  }
#}
line=line[-17]

######## Simulation - run onf IFB----------
...
setwd(wd)
for (sp in 1:16){
  if (file.exists(paste("Data/slim_bottleneck/Input16/testdiv_",species[sp],".txt",sep=""))==T){
    file.remove(paste("Data/slim_bottleneck/Input16/testdiv_",species[sp],".txt",sep=""))
  }
}
lifetime<-as.data.frame(read_excel("Data/SLiM.xlsx",sheet="SLiM"))
species=lifetime[is.na(lifetime$Species_code)==FALSE,]$Species_code
...
lifetime$Max_age_M=as.numeric(lifetime$Max_age_M)
## CONSTRUCT SLIM MODELS
setwd(paste(wd,"/Data/slim_bottleneck/Input16",sep=""))
for (sp in 1:length(species)){
  divforSlim<-readtext("model.txt")
  divforSlim<-strsplit(divforSlim$text,"\n")[[1]]
  # Surv_female
  ...
  #Writefile
  writefile_grep=grep(c("writefile"),divforSlim)
  divforSlim[writefile_grep[1]]=paste(' writeFile("/shared/projects/abc_fish/slim_bottleneck/Output16/div_',species[sp],'_"+K+"_"+iter+".txt", line, append=T);',sep="")
  
  #Writefile
  writefile_grep=grep(c("writefile_popadult"),divforSlim)
  divforSlim[writefile_grep[1]]=paste(' writeFile("/shared/projects/abc_fish/slim_bottleneck/Output16/popadult_',species[sp],'_"+K+"_"+iter+".txt", line, append=T);',sep="")
  
  #Writefile
  writefile_grep=grep(c("writefile_poptotal"),divforSlim)
  divforSlim[writefile_grep[1]]=paste(' writeFile("/shared/projects/abc_fish/slim_bottleneck/Output16/poptotal_',species[sp],'_"+K+"_"+iter+".txt", line, append=T);',sep="")

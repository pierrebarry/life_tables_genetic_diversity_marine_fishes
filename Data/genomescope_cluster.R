...
## LOAD SPECIES INFORMATIONS

species=c("Cgale","Cjuli","Dpunt","Eencr","Dlabr","Hgutt","Lbude","Lmorm","Mmerl","Msurm","Peryt","Scabr","Scant","Scine","Spilc","Ssard","Styph")
loca=c("Li","Mu","Fa","Ga")
num=c("1","2","3","4","5","6")
...
for (i in 1:length(species)){
  ...
  for (j in 1:length(loca)){
    ...
    for (k in 1:length(num)){
      ...
      sample=c(sample,paste(species[i],loca[j],num[k],sep=""))
...
## FONCTION EXTRACT NUMBER
library(stringr)
numextract_point <- function(string){ 
...
## LOAD SUMMARY_GENOMESCOPE

if (file.exists("Summary/Summary_GenomeScope.Rdata")==FALSE){
  
  Summary_GenomeScope=data.frame(Species=c("NA"),Location=c("NA"),Sample=c("NA"),
                                 Heterozygosity_min=c(0),Heterozygosity=c(0),Heterozygosity_max=c(0),
                                 Genome_Length_min=c(0),Genome_Length=c(0),Genome_Length_max=c(0),
                                 Genome_Repeat_min=c(0),Genome_Repeat=c(0),Genome_Repeat_max=c(0),
                                 Genome_Unique_min=c(0),Genome_Unique=c(0),Genome_Unique_max=c(0),
                                 Model_Fit_min=c(0),Model_Fit=c(0),Model_Fit_max=c(0),
                                 Read_error_rate_min=c(0),Read_error_rate=c(0),Read_error_rate_max=c(0))
  
  Summary_GenomeScope[,1]=as.character(Summary_GenomeScope[,1])
  Summary_GenomeScope[,2]=as.character(Summary_GenomeScope[,2])
  Summary_GenomeScope[,3]=as.character(Summary_GenomeScope[,3])
  
} else {
  
  load(file="Summary/Summary_GenomeScope.Rdata")
  
}
  


## GENOME SCOPE FUNCTION
...
info=read.delim(paste(args[[4]],"/summary.txt",sep=""))

one_indiv=data.frame("NAME"=rep(NA,6),"min"=rep(0,6),"max"=rep(0,6))

for (j in 3:8){
  
  if (j==3 | j==7 | j==8){
    
    z=unlist(strsplit(as.character(info[j,1]),"  "))
    z=z[z!=""]
    one_indiv[j-2,1]=z[1]
    one_indiv[j-2,2]=as.numeric(numextract_point(z[2]))
    one_indiv[j-2,3]=as.numeric(numextract_point(z[3]))
    ...
    one_indiv[j-2,2]=as.numeric(paste(a[1],a[2],a[3],a[4],sep=""))
    ...
    one_indiv[j-2,3]=as.numeric(paste(a[1],a[2],a[3],a[4],sep=""))
...
line=c(substr(args[[7]],0,5),substr(args[[7]],6,7),args[[7]],
       one_indiv[1,2],mean(c(one_indiv[1,2],one_indiv[1,3])),one_indiv[1,3],
       one_indiv[2,2],mean(c(one_indiv[2,2],one_indiv[2,3])),one_indiv[2,3],
       one_indiv[3,2],mean(c(one_indiv[3,2],one_indiv[3,3])),one_indiv[3,3],
       one_indiv[4,2],mean(c(one_indiv[4,2],one_indiv[4,3])),one_indiv[4,3],
       one_indiv[5,2],mean(c(one_indiv[5,2],one_indiv[5,3])),one_indiv[5,3],
       one_indiv[6,2],mean(c(one_indiv[6,2],one_indiv[6,3])),one_indiv[6,3])

Summary_GenomeScope=rbind(Summary_GenomeScope,line)

save(Summary_GenomeScope,file="Summary/Summary_GenomeScope.Rdata")
